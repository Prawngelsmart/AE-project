...
ae_data_ts <- ts(ae_data[,2], start = 1, end = 204, frequency = 1)
#view(ae_data_ts)
#plot(ae_data_ts)
#ae_data_ts

ae_data_comp <- ts(ae_data[,2], start = 205, end = 276, frequency = 1)
#view(ae_data_comp)
#plot( ae_data_comp )
...
pred.arima <- predict(unemploymodel, n.ahead=72)
...
#ann prediction
t3 = c(tail(t2,n=1), tail(t1,n=1), tail(result,n=1), residuals_pre[1:69])
t2 = c(tail(t1,n=1), tail(result,n=1), residuals_pre[1:70])
t1 = c(tail(result,n=1), residuals_pre[1:71])
